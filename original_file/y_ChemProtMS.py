...
print("model load")
# ############################################读取数据#################################################################
df_train = pd.read_csv('./data/ChemProtMS/train.tsv', sep='\t')[:1000]
df_dev = pd.read_csv('./data/ChemProtMS/dev.tsv', sep='\t')
df_test = pd.read_csv('./data/ChemProtMS/test.tsv', sep='\t')
...
class YModel(nn.Module):
    def __init__(self, model_name_1, model_name_2, model_name_3, num_labels=5):
        ...
        self.fc_2 = nn.Linear(768, 256)
        self.fc_3 = nn.Linear(768, 256)
        self.attention = nn.MultiheadAttention(embed_dim=256 * 3, num_heads=1) 
        self.MLP_1 = nn.Linear(256 * 3, 64)
        # self.MLP_2 = nn.Linear(128, 16)
        self.MLP_3 = nn.Linear(64, num_labels)
        self.loss_func = nn.CrossEntropyLoss()
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, input_ids_3, attention_mask_3, labels=None):
        ...
        output_1 = self.dr(F.gelu(self.fc_1(output_1[:, 0])))  # output_1 shape: (batch_size, 128)
        output_2 = self.dr(F.gelu(self.fc_2(output_2[:, 0])))  # output_2 shape: (batch_size, 128)
        output_3 = self.dr(F.gelu(self.fc_3(output_3[:, 0])))  # output_3 shape: (batch_size, 128)

        # Concatenate the outputs
        combined_output = torch.cat((output_1, output_2, output_3), dim=1)  # combined_output shape: (batch_size, 128 * 3)

        # Apply fully connected layer
        out = self.dr(F.gelu(self.MLP_1(combined_output)))  
        #out = self.dr(F.gelu(self.MLP_2(out)))  
        logits = self.MLP_3(out)
...
#         self.MLP_1 = nn.Linear(256, 64)
#         self.MLP_3 = nn.Linear(64, num_labels)
#         self.loss_func = nn.CrossEntropyLoss()
#         self.dr = nn.Dropout(0.3)

#     def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, input_ids_3, attention_mask_3, labels=None):
#         output_1 = self.Bert_1(input_ids=input_ids_1, attention_mask=attention_mask_1)[0][:, 0, :]  # output_1 shape: (batch_size, hidden_size)
#         output_2 = self.Bert_2(input_ids=input_ids_2, attention_mask=attention_mask_2)[0][:, 0, :]  # output_2 shape: (batch_size, hidden_size)
#         output_3 = self.Bert_3(input_ids=input_ids_3, attention_mask=attention_mask_3)[0][:, 0, :]  # output_3 shape: (batch_size, hidden_size)

#         # Apply linear transformation
#         output_1 = self.dr(F.gelu(self.fc_1(output_1)))  # output_1 shape: (batch_size, 256)
#         output_2 = self.dr(F.gelu(self.fc_2(output_2)))  # output_2 shape: (batch_size, 256)
#         output_3 = self.dr(F.gelu(self.fc_3(output_3)))  # output_3 shape: (batch_size, 256)

#         # Stack outputs for multihead attention
#         stacked_outputs = torch.stack((output_1, output_2, output_3), dim=0)  # shape: (3, batch_size, 256)

#         # Compute attention
#         attention_output, _ = self.attention(stacked_outputs, stacked_outputs, stacked_outputs)
#         attention_output = attention_output.mean(dim=0)  # Taking mean over the heads

#         # Apply fully connected layer
#         out = self.dr(F.gelu(self.MLP_1(attention_output)))  
#         logits = self.MLP_3(out)
